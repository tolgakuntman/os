...
void datamgr_parse_sensor_files(FILE *fp_sensor_map, FILE *fp_sensor_data) {
    ...
    if(list==NULL) ERROR_HANDLER(true,"error dplist is not allocated");
    ...
    while (fread(&sensor_id, sizeof(sensor_id_t), 1, fp_sensor_data) == 1 &&
           fread(&temperature, sizeof(sensor_value_t), 1, fp_sensor_data) == 1 &&
           fread(&timestamp, sizeof(sensor_ts_t), 1, fp_sensor_data) == 1) {
        sensor_node_data_t temp_sensor;
        temp_sensor.sensor_id = sensor_id;
        int index = dpl_get_index_of_element(list, &temp_sensor);
        if(index==-1)  {fprintf(stderr,
         "Log error: Sensor read has an invalid id %hd\n",
        ...
        if(sensor->reading_count==RUN_AVG_LENGTH){
            ...
            if (sensor->running_avg < SET_MIN_TEMP) {
               fprintf(stderr, "Log error: Room %hd is too cold with average temperature %f°C measured by sensor %hd\n", sensor->room_id, sensor->running_avg,sensor->sensor_id);
            } else if (sensor->running_avg > SET_MAX_TEMP) {
                fprintf(stderr, "Log error: Room %hd is too hot with average temperature %f°C measured by sensor %hd\n", sensor->room_id, sensor->running_avg,sensor->sensor_id);
            }
        }
